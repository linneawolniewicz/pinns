...
size = 1024 # size of r, T, p, and f_boundary
au = 150e6 # 150e6 m/AU
r_limits = [0.4, 120] 
T_limits = [0.001, 1000]
...
# Save data
with open(DATA_PATH + '/f_boundary_' + str(size) + '.pkl', 'wb') as file:
    pkl.dump(f_boundary, file)

with open(DATA_PATH + '/p_' + str(size) + '.pkl', 'wb') as file:
    pkl.dump(p, file)

with open(DATA_PATH + '/T_' + str(size) + '.pkl', 'wb') as file:
    pkl.dump(T, file)

with open(DATA_PATH + '/r_' + str(size) + '.pkl', 'wb') as file:
    pkl.dump(r, file)
    
with open(DATA_PATH + '/J_' + str(size) + '.pkl', 'wb') as file:
    pkl.dump(J, file)
    
with open(DATA_PATH + '/P_predict_' + str(size) + '.pkl', 'wb') as file:
